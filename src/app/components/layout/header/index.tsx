...
const Header = () => {
  ...
  {/* Menu Button - always visible when menu is closed */}
  {!menuOpen && !isClosing && (
  ...
  {(menuOpen || isClosing) && (
    <div
      ...
      onTransitionEnd={() => {
        if (isClosing) {
          setMenuOpen(false); // fully remove menu
          setIsClosing(false);
        }
      }}
    >
      ...
      <div className="flex items-center justify-between pb-5 border-b border-secondary/15 dark:border-white/15">
        <p className="text-[#181f22] dark:text-white text-4xl">Menu</p>
        <div
          onClick={() => setIsClosing(true)} // close with animation
        ...
        <ul className="flex flex-col gap-2 pb-4">
          {menuData?.map((menuItem: any, index: number) => (
            <MenuList
              key={index}
              item={menuItem}
              closeMenu={() => setIsClosing(true)} // smooth close
            />
          ))}
        </ul>
      </div>
      ...
      <div>
        ...
        <Link href="mailto:earnova2@gmail.com">
          <h4>Earnova2@gmail.com</h4>
        </Link>
      </div>
    </div>
  )}
